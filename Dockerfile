...
RUN curl -LO https://github.com/neovim/neovim/releases/download/v0.12.1/nvim-linux-x86_64.tar.gz \
...
RUN nvim --headless -c 'lua require("nvim-treesitter.install").install({ "dockerfile", "yaml" }, {sync = true})' -c 'quitall' 2>&1 || true
...
RUN nvim --headless -c 'lua require("nvim-treesitter.install").install({ "go", "dockerfile", "yaml" }, {sync = true})' -c 'quitall' 2>&1 || true
...
RUN nvim --headless -c 'lua require("nvim-treesitter.install").install({ "php", "dockerfile", "yaml" }, {sync = true})' -c 'quitall' 2>&1 || true
